class ServiceCategoriesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @service_category.update(service_category_params)
        format.html { redirect_to @service_category, notice: "Service category was successfully updated.", status: :see_other }
  ...
  private
    ...
    def set_service_category
      @service_category = ServiceCategory.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def service_category_params
      params.expect(service_category: [ :name, :description, :active ])
    end
end
